...
export function parseCallStatement(state: ParserContext): CallStatementNode {
  ...
  if (!state.checkAny(allowed)) throw parserError(state, state.peek(), ERR_EXPECTED_CALL_NAME)
  state.advance()
  ...
  // normalize callable name: consider keyword tokens mapped to their canonical names
  let callableLexeme = nameToken.lexeme
  if (nameToken.type === TokenType.FinDeSecuencia) callableLexeme = 'FinDeSecuencia'
  if (nameToken.type === TokenType.NoFinDeSecuencia) callableLexeme = 'NoFinDeSecuencia'
  if (nameToken.type === TokenType.Arrancar) callableLexeme = 'Arrancar'
  if (nameToken.type === TokenType.Crear) callableLexeme = 'Crear'
  if (nameToken.type === TokenType.Avanzar) callableLexeme = 'Avanzar'
  if (nameToken.type === TokenType.Cerrar) callableLexeme = 'Cerrar'

  return {
    type: 'CallStatement',
      call: {
      type: 'FunctionCall',
      name: normalizeCallableName(callableLexeme),
      arguments: args,
      line: nameToken.line,
...
function normalizeCallableName(name: string) {
  const low = name.toLowerCase()
  if (low === 'fds') return 'FinDeSecuencia'
  if (low === 'nfds') return 'NoFinDeSecuencia'
  return name
}
